...
__all__ = ['JSONLD', 'InlineJS', 'Script', 'Scripts']
    ...
    def _escaped_parameters(self, params):
        ...
        for key, value in (params or {}).items():
            if isinstance(value, (bool, NoneType)):
                ...
                escaped_params[key] = as_safe_json(value)
...
class JSONLD(object):
    ...
    def render(self):
        js = as_safe_json(self.params)
        return u'<script type="application/ld+json">%s</script>' % js

    def __repr__(self):
        return 'JSONLD(%r, key=%r)' % (self.params, self.key)
