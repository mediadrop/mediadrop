...
Most of MediaCore's own StorageEngines inherit from the helpful subclasses
...
There are three components in MediaCore's file storage system:
...
When a new MediaFile is being added to MediaCore (for example, via the admin
...
MediaCore will attempt to find the most appropriate StorageEngine to handle the
...
precedence graph. MediaCore finds a topological ordering according to these
...
A new StorageEngine can be added to MediaCore simply by subclassing
